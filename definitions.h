...
//Tenemos el indice de los vertices
...
//Ejemplo con 5 vertices:
    //num_vert:5
    //Un puntero a la tabla de vertices (Que tendra 5 elementos)
typedef struct elm_matriz{
    double Matriz[16];
    struct elm_matriz *mptr;  
...
struct object3d{
    ...
    //Un puntero a la tabla de vertices (Que tendra 5 elementos)     
        //Dentro de la tabla de vertices encontramos cada vertice con sus coordenadas.
        //Tambien junto con sus coordenadas encontraremos el numero de poligonos en los que aparece cada vertice
    vertex *vertex_table;               /* table of vertices */
    //num_faces:5
    GLint num_faces;                    /* number of faces in the object */
    //Un puntero a la tabla de poligonos (5 otra vez)
        //En esta tabla encontraremos dos campos: 
        //Numero de vertices que tiene el poligonos
        //Un puntero a una tabla que contiene los vertices que conforman nuestro poligono (Ejemplo de piramide: El primero apuntara a [0,2,3] que son los vertices que conforman una cara)
    //CUIDADO: El orden de la tabla (Tecnica rara del dedo) y que siempre se empieza en el 0 no en el 1
